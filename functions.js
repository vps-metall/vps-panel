...
async function asyncWriteToFirebase(path, data) {
    const ref = database.ref(path);
    try {
        const snapshot = await ref.once('value');
        if (snapshot.exists()) {
            throw new Error(`Данные по пути "${path}" уже существуют.`);
        } else {
            await ref.set(data);
            return true;
        }
...
async function asyncOverwriteToFirebase(path, data) {
    const ref = database.ref(path);

    try {
        await ref.set(data); // Ждём, пока Firebase не выполнит запись
        return true;  // Возвращаем успех, если запись прошла
    } catch (error) {
...
// асинхронная функция для чтения с бд
async function asyncReadToFirebase(path) {
    const ref = database.ref(path);

    try {
        const snapshot = await ref.once('value');  // Ждем результата чтения из Firebase
        if (snapshot.exists()) {
            return snapshot.val();  // Возвращаем данные, если они существуют
        } else {
...
async function searchByKeyInFirebase(path, searchString) {
    const ref = database.ref(path);
    ...
    try {
        ...
        const data = snapshot.val(); // объект: { key1: value1, key2: value2, ... }

        // Фильтрация по ключам:
        const result = Object.entries(data)
            .filter(([key, _]) => key.includes(searchString))
        ...
        console.error('Ошибка при поиске по ключам в Firebase:', error);
        return null;
    }
}

// асинхронная функция для чтения с бд первых N елементов
async function asyncReadFirstNItems(path, limit) {
    const ref = database.ref(path)
        .orderByKey()
        .limitToFirst(limit);
...
function readFirebaseByKeyRange(path, startKey, endKey, callback) {
    ...
    // Асинхронная Функция для однократного получения данных в диапазоне ключей с Firebase
async function asyncReadFirebaseByKeyRange(path, startKey, endKey) {
    try {
        const ref = database.ref(path);
...
function getMonthsInRange(startDateMs, endDateMs) {
 ...
 // из Date.now() получает название месяца на укр рус год и начало и конец месяца
function getMonthInfo(dateNowTimestamp) {
  const date = new Date(dateNowTimestamp);

  const year = date.getFullYear();
  const monthIndex = date.getMonth(); // от 0 (январь) до 11 (декабрь)
  ...
  // Конец месяца: последний день в 23:59:59.999
  const endOfMonth = new Date(year, monthIndex + 1, 0, 23, 59, 59, 999).getTime();

  return {
    year: year,
    mounthNameUa: monthNamesUa[monthIndex],
    mounthNameRu: monthNamesRu[monthIndex],
...
//возаращает date now прошлого месяца
function getPreviousMonth(dateNow) {
    const date = new Date(dateNow);
    // 0-й день текущего месяца = последний день прошлого месяца
    const endPrevMonth = new Date(date.getFullYear(), date.getMonth(), 0, 23, 59, 59, 999);
    return endPrevMonth.getTime();
}
//возаращает date now следующего месяца
function getNextMonth(dateNow) {
    const date = new Date(dateNow);
    const startNextMonth = new Date(date.getFullYear(), date.getMonth() + 1, 1, 0, 0, 0, 0);
    return startNextMonth.getTime();
}

function getYearBounds(timestamp) {
...
function formatedInputDate(idInput){
    const input = document.querySelector(`#${idInput}`); // замени на свой id
    const rawDate = input.value; // формат yyyy-mm-dd
...
function getFormattedDateForGrafik(timestamp) {
    const date = new Date(timestamp);
    const year = date.getFullYear();
...
function formatDateNowToInputDateFormat(timestamp) {
  const date = new Date(timestamp);
...
function getDatesInRangeToInputDateFormat(startTimestamp, endTimestamp) {
    // Проверяем, что переданные временные метки — числа
    if (typeof startTimestamp !== 'number' || typeof endTimestamp !== 'number') {
        console.error('Ошибка: Временные метки должны быть числами');
        return [];
    }

    // Проверим, что временные метки валидны
    const startDate = new Date(startTimestamp);
    const endDate = new Date(endTimestamp);

    // Если startDate или endDate некорректны, выведем ошибку
    if (isNaN(startDate) || isNaN(endDate)) {
        console.error('Ошибка: Невалидная временная метка');
        return [];
    }

    console.log(`startDate: ${startDate.toISOString()}, endDate: ${endDate.toISOString()}`);

    // Проверим, что startDate меньше или равен endDate
    if (startDate > endDate) {
        console.log('Ошибка: startDate больше endDate');
        return [];
    }

    const dates = [];

    // Пройдём по всем дням в диапазоне
